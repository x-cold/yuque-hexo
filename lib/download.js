...
class Downloader {
  ...
  generatePost(post) {
    ...
    const text = ejs.render(template, {
      title,
      date: date || formatDateWithTimeZone(created_at, this.yuqueConfig.timezone),
...
function formatRaw(body) {
  ...
  return body.replace(multiBr, '<br>');
}
...
function formatDateWithTimeZone(date, timezone) {
  return moment(date).tz(timezone || 'Asia/Shanghai').format('YYYY-MM-DD HH:mm:ss');
}
